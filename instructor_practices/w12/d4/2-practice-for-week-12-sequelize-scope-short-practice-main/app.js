...
app.get('/stores/:storeId/instruments', async (req, res, next) => {
    const filterStoreInstruments = await Instrument.scope({ method: ['byStoreId', req.params.storeId] }, 'defaultScope').findAll()
    // Your code here
    res.json(filterStoreInstruments);
});

app.get('/stores/:storeId/instruments/:type', async (req, res, next) => {
    const filteredInstruments = await Instrument.scope({method: ['byStoreId', req.params.storeId, req.params.type]}).findAll()
    // Your code here
